...
class cronTest extends TestCase
{
    ...
    {
        echo "\n" . __CLASS__ . '::' . __FUNCTION__ . ' : ';
        $cron1 = cron::byClassAndFunction('calendar', 'pull', ['event_id' => intval(1)]);
        if (!is_object($cron1)) {
            $cron1 = new cron();
            $cron1->setClass('calendar');
            $cron1->setFunction('pull');
            $cron1->setOption(['event_id' => intval(1)]);
            $cron1->setLastRun(date('Y-m-d H:i:s'));
        }
        $cron1->setSchedule('00 00 * * * 2020');
        $cron1->save();

        $cron2 = cron::byClassAndFunction('calendar', 'pull', ['event_id' => intval(2)]);
        if (!is_object($cron2)) {
            $cron2 = new cron();
            $cron2->setClass('calendar');
            $cron2->setFunction('pull');
            $cron2->setOption(['event_id' => intval(2)]);
            $cron2->setLastRun(date('Y-m-d H:i:s'));
        }
        $cron2->setSchedule('00 00 * * * 2020');
        $cron2->save();
        ...
        $cron3 = cron::byClassAndFunction('calendar', 'pull', ['event_id' => intval(1)]);
        if (!is_object($cron3)) {
            $cron3 = new cron();
            $cron3->setClass('calendar');
            $cron3->setFunction('pull');
            $cron3->setOption(['event_id' => intval(1)]);
