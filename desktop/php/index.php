...
if (init('rescue', 0) == 0) {
	...
	if (count($plugins_list) > 0) {
		foreach ($plugins_list as $category_name => $category) {
			...
			$plugin_menu .= '<li class="dropdown-submenu"><a class="dropdown-item dropdown-toggle"><i class="fas ' . $icon . '"></i> {{' . $name . '}}</a>';
			...
			foreach ($category as $pluginList) {
				...
				$plugin_menu .= '<li style="padding-right:10px"><a class="dropdown-item" href="index.php?v=d&m=' . $pluginList->getId() . '&p=' . $pluginList->getIndex() . '"><img class="img-responsive" style="width : 20px;display:inline-block;" src="' . $pluginList->getPathImgIcon() . '" /> ' . $pluginList->getName() . '</a></li>';
				if ($pluginList->getDisplay() != '' && config::byKey('displayDesktopPanel', $pluginList->getId(), 0) != 0) {
					$panel_menu .= '<li style="padding-right:10px"><a class="dropdown-item"  href="index.php?v=d&m=' . $pluginList->getId() . '&p=' . $pluginList->getDisplay() . '"><img class="img-responsive" style="width : 20px;display:inline-block;" src="' . $pluginList->getPathImgIcon() . '" /> ' . $pluginList->getName() . '</a></li>';
				}
...
if (!isConnect()) {
	...
	?>
		...
		?>
			<header class="reportModeHidden navbar navbar-expand-md navbar-dark fixed-top bg-dark" style="height: 56px;">
				...
				<ul class="navbar-nav mr-auto">
					<li class="dropdown nav-item">
						<a class="dropdown-toggle nav-link" data-toggle="dropdown"><i class="fas fa-home"></i> <span class="hidden-xs hidden-sm hidden-md">{{Accueil}}</span> <b class="caret"></b></a>
						<ul class="dropdown-menu">
							<li class="dropdown-submenu">
								<a class="dropdown-item dropdown-toggle" id="bt_gotoDashboard" href="index.php?v=d&p=dashboard"><i class="fas fa-tachometer-alt"></i> {{Dashboard}}</a>
...
foreach (jeeObject::buildTree(null, false) as $object_li) {
			echo '<li><a class="dropdown-item" href="index.php?v=d&p=dashboard&object_id=' . $object_li->getId() . '">' . $object_li->getHumanName(true) . '</a></li>';
		}
		?>
								</ul>
							</li>
							<li class="dropdown-submenu">
								<a class="dropdown-item dropdown-toggle" id="bt_gotoView"><i class="fas fa-picture-o"></i> {{Vue}}</a>
								<ul class="dropdown-menu scrollable-menu" role="menu" style="height: auto;max-height: 600px; overflow-x: hidden;">
									<?php
foreach (view::all() as $view_menu) {
			echo '<li><a class="dropdown-item" href="index.php?v=d&p=view&view_id=' . $view_menu->getId() . '">' . trim($view_menu->getDisplay('icon')) . ' ' . $view_menu->getName() . '</a></li>';
		}
		?>
								</ul>
							</li>
							<li class="dropdown-submenu">
								<a class="dropdown-item dropdown-toggle" id="bt_gotoPlan"><i class="fas fa-paint-brush"></i> {{Design}}</a>
								<ul class="dropdown-menu scrollable-menu" role="menu" style="height: auto;max-height: 600px; overflow-x: hidden;">
									<?php
foreach (planHeader::all() as $plan_menu) {
			echo '<li><a class="dropdown-item" href="index.php?v=d&p=plan&plan_id=' . $plan_menu->getId() . '">' . trim($plan_menu->getConfiguration('icon') . ' ' . $plan_menu->getName()) . '</a></li>';
		}
		?>
								</ul>
							</li>
							<li class="dropdown-submenu">
								<a class="dropdown-item dropdown-toggle" id="bt_gotoPlan3d"><i class="fas fa-cubes"></i> {{Design 3D}}</a>
								<ul class="dropdown-menu scrollable-menu" role="menu" style="height: auto;max-height: 600px; overflow-x: hidden;">
									<?php
foreach (plan3dHeader::all() as $plan3d_menu) {
			echo '<li><a class="dropdown-item" href="index.php?v=d&p=plan3d&plan3d_id=' . $plan3d_menu->getId() . '">' . trim($plan3d_menu->getConfiguration('icon') . ' ' . $plan3d_menu->getName()) . '</a></li>';
		}
		?>
								</ul>
							</li>
...
echo $panel_menu;
		?>
					...
					<li class="dropdown nav-item">
						<a class="dropdown-toggle nav-link" data-toggle="dropdown"><i class="fas fa-stethoscope"></i> <span class="hidden-xs hidden-sm hidden-md">{{Analyse}}</span> <b class="caret"></b></a>
						<ul class="dropdown-menu" role="menu">
							<li><a class="dropdown-item" href="index.php?v=d&p=history"><i class="fas fa-bar-chart-o"></i> {{Historique}}</a></li>
							<?php
if (isConnect('admin')) {
			?>
								<li><a class="dropdown-item" href="index.php?v=d&p=report"><i class="fas fa-newspaper-o"></i> {{Rapport}}</a></li>
								<?php
}
		?>
							<li class="divider"></li>
							<li><a class="dropdown-item" href="#" id="bt_showEventInRealTime"><i class="fas fa-tachometer-alt"></i> {{Temps réel}}</a></li>
							<?php
if (isConnect('admin')) {
			?>
								<li><a class="dropdown-item" href="#" id="bt_showNoteManager"><i class="fas fa-sticky-note"></i> {{Note}}</a></li>
								<li><a class="dropdown-item" href="index.php?v=d&p=log"><i class="far fa-file"></i> {{Logs}}</a></li>
								<li><a class="dropdown-item" href="index.php?v=d&p=eqAnalyse"><i class="fas fa-battery-full"></i> {{Equipements}}</a></li>
								<li class="divider"></li>
								<li><a class="dropdown-item" href="index.php?v=d&p=health"><i class="fas fa-medkit"></i> {{Santé}}</a></li>
								<?php
}
		?>
						</ul>
...
if (isConnect('admin')) {
			?>
						<li class="dropdown nav-item">
							<a class="dropdown-toggle nav-link" data-toggle="dropdown"><i class="fas fa-wrench"></i> <span class="hidden-xs hidden-sm hidden-md">{{Outils}}</span> <b class="caret"></b></a>
							<ul class="dropdown-menu" role="menu">
								<li><a class="dropdown-item" href="index.php?v=d&p=object"><i class="fas fa-picture-o"></i> {{Objets}}</a></li>
								<li><a class="dropdown-item" href="index.php?v=d&p=interact"><i class="far fa-comments"></i> {{Interactions}}</a></li>
								<li><a class="dropdown-item" href="index.php?v=d&p=display"><i class="fas fa-th"></i> {{Résumé domotique}}</a></li>
								<li><a class="dropdown-item" href = "index.php?v=d&p=scenario"><i class = "fa fa-cogs"></i> {{Scénarios}}</a></li>
							</ul>
						</li>
						<li class="dropdown nav-item">
							<a class="nav-link dropdown-toggle" data-toggle="dropdown"><i class="fas fa-tasks"></i> <span class="hidden-xs hidden-sm hidden-md">{{Plugins}}</span> <b class="caret"></b></a>
							<ul class="dropdown-menu" role="menu">
								<li><a class="dropdown-item" href="index.php?v=d&p=plugin"><i class="fas fa-tags"></i> {{Gestion des plugins}}</a></li>
				...
				<ul class="navbar-nav my-2">
					<li>
						<a class="nav-link" href="#" style="cursor:default;">
							<?php
echo jeeObject::getGlobalHtmlSummary();
		?>
						</a>
					</li>
					<?php
$nbMessage = message::nbMessage();
		$displayMessage = ($nbMessage > 0) ? '' : 'display : none;';?>
					<li class="nav-item">
						<a class="nav-link" href="#" id="bt_messageModal">
		...
		$displayUpdate = ($nbUpdate > 0) ? '' : 'display : none;';?>
					<li class="nav-item">
						<a class="nav-link" href="index.php?v=d&p=update">
			...
			?>
								<li class="dropdown">
									<a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#"><i class="fas fa-cogs"></i><span class="caret"></span></a>
									<ul class="dropdown-menu">
										<li><a class="dropdown-item" href="index.php?v=d&p=administration" tabindex="0"><i class="fas fa-wrench"></i> {{Configuration}}</a></li>
										<li><a class="dropdown-item" href="index.php?v=d&p=backup"><i class="fas fa-floppy-o"></i> {{Sauvegardes}}</a></li>
										<li><a class="dropdown-item" href="index.php?v=d&p=update"><i class="fas fa-refresh"></i> {{Centre de mise à jour}}</a></li>
										<li><a class="dropdown-item" href="index.php?v=d&p=cron"><i class="fas fa-tasks"></i> {{Moteur de tâches}}</a></li>
										<li><a class="dropdown-item" href="index.php?v=d&p=custom"><i class="fas fa-pencil-alt"></i> {{Personnalisation avancée}}</a></li>
										<li role="separator" class="divider"></li>
										<li><a class="dropdown-item" href="index.php?v=d&p=user"><i class="fas fa-users"></i> {{Utilisateurs}}</a></li>
									</ul>
								</li>
							<?php }
		?>
							<li class="dropdown nav-item">
								...
								<ul class="dropdown-menu">
									<li><a class="dropdown-item" href="index.php?v=d&p=profils"><i class="fas fa-briefcase"></i> {{Profil}} <?php echo $_SESSION['user']->getLogin(); ?></a></li>
									<li class="divider"></li>
									<li><a class="dropdown-item" href="index.php?v=m" class="noOnePageLoad"><i class="fas fa-mobile"></i> {{Version mobile}}</a></li>
									<li class="divider"></li>
									<li><a class="dropdown-item" href="#" id="bt_jeedomAbout"><i class="fas fa-info-circle"></i> {{Version}} v<?php echo jeedom::version(); ?></a></li>
									<?php	if (jeedom::isCapable('sudo') && isConnect('admin')) {
			echo '<li class="divider"></li>';
			echo '<li class="cursor"><a class="dropdown-item" id="bt_rebootSystem" state="0"><i class="fas fa-repeat"></i> {{Redémarrer}}</a></li>';
			echo '<li class="cursor"><a class="dropdown-item" id="bt_haltSystem" state="0"><i class="fas fa-power-off"></i> {{Eteindre}}</a></li>';

		}
		?>
									<li class="divider"></li>
									<li><a class="dropdown-item" href="index.php?v=d&logout=1" class="noOnePageLoad"><i class="fas fa-sign-out-alt"></i> {{Se déconnecter}}</a></li>
								</ul>
							</li>
							<li class="nav-item">
								<a id="bt_getHelpPage" class="nav-link" data-plugin="<?php echo init('m'); ?>" data-page="<?php echo init('p'); ?>" title="{{Aide sur la page en cours}}"><i class="fas fa-question-circle" ></i></a>
							</li>
							<?php if (isConnect('admin')) {
			?>
								<li class="nav-item">
				...
				?>
										<a target="_blank" class="nav-link" href="<?php echo $plugin->getIssue() ?>" title="{{Envoyer un rapport de bug}}">
											<i class="fas fa-exclamation-circle" ></i>
										</a>
									<?php } else {?>
										<a class="bt_reportBug nav-link" title="{{Envoyer un rapport de bug}}">
											<i class="fas fa-exclamation-circle" ></i>
										</a>
									<?php }?>
								</li>
