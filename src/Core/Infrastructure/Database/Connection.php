...
namespace Jeedom\Core\Infrastructure\Database;
...
class Connection
{
    ...
    public static function getConnection()
    {
        ...
        } else if (self::$sharedInstance->lastConnection + 120 < time()) {
            try {
    ...
    {
        $tables = self::Prepare('SELECT TABLE_NAME FROM information_schema.TABLES WHERE Data_Free > 0', [], self::FETCH_TYPE_ALL);
        foreach ($tables as $table) {
            $table = array_values($table);
            $table = $table[0];
            self::Prepare('OPTIMIZE TABLE `' . $table . '`', []);
        }
    }
    ...
    public static function save($object, $_direct = false, $_replace = false)
    {
        ...
        if (!self::getField($object, 'id')) {
            ...
            $res = self::Prepare($sql, $parameters);
    ...
    public static function refresh($object)
    {
        ...
        $parameters = ['id' => self::getField($object, 'id')];
        ...
        $sql .= '1';
        $newObject = self::Prepare($sql, $parameters, self::FETCH_TYPE_ROW, \PDO::FETCH_CLASS, get_class($object));
     ...
     * @param object $_object Objet sur lequel appliquer les filtres
     *
     * @return object|object[] Objet ou liste d'objets correspondant à la requête
    ...
    public static function lock($object) {
        ...
        list($void, $parameters) = self::buildQuery($object);
        ...
        $res = self::Prepare($sql, $parameters, self::FETCH_TYPE_ROW);
